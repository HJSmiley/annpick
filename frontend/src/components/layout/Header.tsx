        ...
        <div className="flex justify-between items-center h-full">
          <div className="flex items-center space-x-8">
            ...
            <Link to="/" className="flex items-center">
              <img src="/images/anpicktest.svg" alt="앤픽 로고" className="w-[80px] h-[40px]"/>
            </Link>
            {/* 네비게이션 메뉴 */}
            <nav>
              <ul className="flex space-x-6">
                <li>
                  <Link
                    to="/"
                    className={`font-black text-xl transition-colors ${isActive('/') ? 'text-orange-500' : ''}`}
                    style={{
                      color: isActive('/') ? 'rgb(249, 115, 22)' : getTextColor(false), // 활성 링크는 주황색, 아니면 동적 색상
                    }}
                  >
                ...
                <li>
                  <Link
                    to="/anime-search"
                    className={`font-bold text-xl transition-colors ${isActive('/anime-search') ? 'text-orange-500' : ''}`}
                    style={{
                      color: isActive('/anime-search') ? 'rgb(249, 115, 22)' : getTextColor(false),
                    }}
                  >
